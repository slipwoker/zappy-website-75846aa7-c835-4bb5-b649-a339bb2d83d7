document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, rule) {
      const label = field.closest('[class*="field"], [class*="group"], label, div')
    ...
    function validateField(field) {
      ...
      if (value.trim() !== '') {
        ...
        if ((type === 'tel') && !VALIDATORS.tel(value)) {
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      const successMsg = contactForm.querySelector('.form-success, [class*="success"]');
  ...
  const OBSERVE_TARGETS = '[class*="fade-in"], [class*="animate"], [class*="scroll-reveal"], .section, section > *, .card, [class*="card"], [class*="feature"], [class*="service"], [class*="testimonial"]';

  const animatableElements = document.querySelectorAll(OBSERVE_TARGETS);

  if (animatableElements.length > 0 && 'IntersectionObserver' in window) {
  ...
  } else if (animatableElements.length > 0) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "פרח בגני משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '75846aa7-c835-4bb5-b649-a339bb2d83d7',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '75846aa7-c835-4bb5-b649-a339bb2d83d7';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '75846aa7-c835-4bb5-b649-a339bb2d83d7';
